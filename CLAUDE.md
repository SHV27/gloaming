...
- v4.0 (Session 4 — "Ship & Shine", final) — **Juice:** procedural Web-Audio SFX (ambient drone, Dread-paced heartbeat, Hollow stinger, Ward chime, Haunt sting, warm/sour push-luck tones; global mute; starts on first gesture); fx store + FxLayer (screenshake + zoom-punch + flash) wired to beats via a reactive `GameFx` diff hook; ambient embers; ember/mote particle bursts; gliding tokens; press feedback on every button. **Teaches itself:** Guided-Game toggle → the board coaches the first turns in-voice gated to the legal action; contextual JIT tips (first Tainted/Hollow/Ward); always-available How-to-Play. **Spread/ship:** Ledger → downloadable PNG run-card (canvas + Web Share); OG/Twitter meta + favicon + og art; error boundary (in-world fallback); migration-safe persistence `merge` guard; touch hit-areas + keyboard-movable nodes + ARIA; reduced-motion honored; lazy-loaded overlays (code-split). **Monetization-ready (no live payments, no dark patterns):** data-driven cosmetic board themes that never touch play + local profile/stats; 3 themes unlocked by playing; `MONETIZATION.md`. Added `LICENSE` (MIT).
- v3.0 (Session 3 — "Depth & Identity") — Added 4 data-driven Survivor roles (Lampwright/Cartographer/Warden/Forsaken) with a lobby character-select, sigils/colors, contextual first-use teaching, and narrator name-drops. Turned the Haunt into a real deck of 6 scenarios with mechanics (Collector: Hunt doubles + Hollows steal Light; Flood: Gloom advances twice + Wards decay; Mimic: one false Ward, Heart opens on the 2 true; Long Night: Dread frozen, board Tainted, creeping flood — survivable sprint; Famine: Lantern leaks; The Gate is Narrow: finite escape). Wired finite-escape resolution (only those on the Heart at Ritual's end get out, up to Gate capacity) and the Whisper (finite-only, 3–4P, targets Forsaken→Marked→random). Upgraded the Ledger to a structured roster (role + fate + verdict per player) + fable. Cartographer foresight previews the Gloom's next spread via a shared pure `gloom.ts` (single source of truth for advance + preview). Fixed a Long-Night soft-lock (Heart could never flood → infinite stall). Save bumped to v3. Deferred to S4: juice/sound, self-teaching tutorial, shareable Ledger card, virality/monetization.
- v2.0 (Session 2) — `GLOAMING_RULEBOOK.md` is now the source of truth. Reworked from "abstract graph" into a real game: legible goal (3 Wards → Heart opens → Escape Ritual), 3-state nodes (Lit/Tainted/Flooded), living Gloom tide visual, the Hollow (telegraphed pathfinding predators), push-your-luck Search (press/bank, 2nd-omen collapse), wounds→Marked→Claimed, loud Dread clock (vignette/desaturate/heartbeat), aware narrator, intro + always-on objective tracker + first-time tooltips. Save bumped to v2. Deferred to S3: Survivor roles, full Haunt scenario-deck rule-rewrites, Marked-targeted Whisper. (See DESIGN.md "v2 VERDICT".)
